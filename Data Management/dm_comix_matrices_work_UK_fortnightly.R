...
#filter out participants with more than 750 contacts
pt <- pt[part_id != "10908"]
pt <- pt[part_id != "602622"]
pt <- pt[part_id != "501989"]
pt <- pt[part_id != "601325"]
pt <- pt[part_id != "604899"]
...
e_fortnights_frame$dominant_eigenvalue <- sapply(e_fortnights_frame$dominant_eigenvalue, as.numeric)

#save dominant eigenvalues
qs::qsave(e_fortnights_frame, file.path(data_path, "comix_eigens_work_UK_fortnightly.qs"))
