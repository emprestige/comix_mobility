##compare data for the Netherlands 
...
#load data
cnts <- qs::qread(file.path(data_path, "part_cnts_work_NL.qs"))
cnts <- cnts[date <= ymd("2021-03-24")]
cnts_longer <- qs::qread(file.path(data_path, "part_cnts_work_NL_longer.qs"))
...
cnts_longer <- cnts_longer[date > "2021-03-24"]
...
qs::qsave(cnts_all, file.path(data_path, "part_cnts_work_NL_all.qs"))

##other

#load data
cnts <- qs::qread(file.path(data_path, "part_cnts_other_NL.qs"))
cnts <- cnts[date <= ymd("2021-03-24")]
cnts_longer <- qs::qread(file.path(data_path, "part_cnts_other_NL_longer.qs"))
cnts_longer[, sample_type := ifelse(part_age == "[0,1)" | part_age == "[1,6)" | part_age == "[6,12)" | part_age == "[12,18)", "child", "adult")] 
cnts_longer[, date := as.Date(parse_date_time(sday_id, orders = "ymd"))]
...
qs::qsave(cnts_all, file.path(data_path, "part_cnts_other_NL_all.qs"))
