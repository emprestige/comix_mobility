...
ct_work <- rbind(cnts_nonmass, ct_mass_groups_work)
...
ct_other <- rbind(cnts_nonmass, ct_mass_groups_other)
...
ct_work <- as.data.table(rlang::duplicate(ct_work))
ct_other <- as.data.table(rlang::duplicate(ct_other))
...
ct_p_work <- ct_p_work[, sample_type := ifelse(part_age == "[0,1)" | part_age == "[1,4)" | part_age == "[5,11)" | part_age == "[12,15)" | part_age == "[16,17)", "child", "adult")] 
ct_p_other <- merge(ct_other, pt_dates, by = c("part_id"), all.x = TRUE)
ct_p_other <- ct_p_other[, sample_type := ifelse(part_age == "[0,1)" | part_age == "[1,4)" | part_age == "[5,11)" | part_age == "[12,15)" | part_age == "[16,17)", "child", "adult")] 
...
#save as qs file
qs::qsave(pt_cnt_work, file.path(dir_data_validate, "part_cnts_work_BE_longer.qs"))
qs::qsave(pt_cnt_other, file.path(dir_data_validate, "part_cnts_other_BE_longer.qs"))
