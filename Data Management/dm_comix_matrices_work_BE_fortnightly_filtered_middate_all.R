...
ct <- qs::qread(file.path(data_path, "contact_work_BE.qs"))

#match column name for function
pt$dayofweek <- pt$weekday

#filter to relevant dates 
pt_date <- pt[date <= ymd("2021-03-04")]
ct_date <- ct[date <= ymd("2021-03-04")]
...
pt <- pt[, part_age := NULL]
ct <- qs::qread(file.path(data_path, "contact_work_BE_longer.qs"))
...
e_middates_frame <- e_middates_frame[order(mid_date)]
#e_middates_frame$mid_date <- as.Date(e_middates_frame$mid_date, origin = "1970-01-01")

#save dominant eigenvalues
qs::qsave(e_middates_frame, file.path(data_path, "comix_eigens_work_BE_fortnightly_filtered_middate_all.qs"))
