...
test.plot1 <- ggplot(data = work.plus, aes(x = mid_date)) +
  ...
  geom_vline(aes(xintercept = ifelse(is.na(special), NA, mid_date)), linetype = 4) +
  geom_text(aes(x = ymd("2020-04-12"), y = +Inf, vjust = 2, label = "Easter")) +
  geom_text(aes(x = ymd("2020-08-02"), y = +Inf, vjust = 2, label = "Summer Hols")) +
  geom_text(aes(x = ymd("2020-12-20"), y = +Inf, vjust = 2, label = "Xmas/NYE")) +
  geom_text(aes(x = ymd("2021-04-04"), y = +Inf, vjust = 2, label = "Easter")) +
  geom_text(aes(x = ymd("2021-08-08"), y = +Inf, vjust = 2, label = "Summer Hols")) +
  geom_text(aes(x = ymd("2021-12-26"), y = +Inf, vjust = 2, label = "Xmas/NYE")) +
  scale_x_date(labels = date_format("%B-%Y")) + 
  scale_y_continuous(limits = c(0, 1.25), breaks = seq(0, 1.25, by = 0.5),
                     name = "Measure of Mixing \nwithin Work") +
  # sec.axis = sec_axis(trans = ~ .,
  #                     name = "Google Mobility ''Workplaces'' Visits")) +
  labs(x = "Date", fill = "Status", linetype = "Data Type") +
  scale_fill_manual(values = c("No restrictions" = "#009E73", 
...
#plot work and other 
ggarrange(test.plot1, test.plot2, common.legend = T, ncol = 1, nrow = 2,
          legend = "bottom", labels = "AUTO")
ggarrange(work_pval_cor, other_pval_cor, common.legend = F, 
          legend = "bottom", labels = "AUTO")
>>>>>>> Stashed changes
