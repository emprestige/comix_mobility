...
weighted_means <- num_merge[, .(study, work = weighted.mean(work, day_weight),
                  ...
                  by = .(fortnight = paste(isoyear(date), "/", 
...
qs::qsave(ests, file.path(data_path, "scaling_factors_BE_fortnightly.qs"))
write.csv(ests, file.path(data_path, "scaling_factors_BE_fortnightly.csv"))
