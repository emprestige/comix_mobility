##residential mobility and non-home contacts
...
poly <- poly[, .(week = 0, study, status, nonhome = mean(nonhome, na.rm = T))]
poly <- unique(poly)
...
mob_cnt <- mob_cnt[, .(week, study, status, nonhome, 
                       nonhome_frac = nonhome/head(nonhome, 1),
                       residential)]

#plot mean
plh <- ggplot(mob_cnt) + 
  geom_point(aes(x = residential, y = nonhome_frac, colour = status)) + 
  #ylim(0, 1) + xlim(0, 1) +
