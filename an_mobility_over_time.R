...
gm_av <- gm2[, .(retail = mean(retail_recreation), 
             ...
             by = .(month = paste(year(date), "/", month(date)))]

#plot retail
retail <- ggplot(gm_av) + 
  geom_line(aes(month, retail), group = 1) + 
  guides(x = guide_axis(angle = 90))
retail

#plot grocery
grocery <- ggplot(gm_av) +
  geom_line(aes(month, grocery), group = 1) + 
  guides(x = guide_axis(angle = 90))
grocery

#plot parks
parks <- ggplot(gm_av) + 
  geom_line(aes(month, parks), group = 1) + 
  guides(x = guide_axis(angle = 90))
parks

#plot transit
transit <- ggplot(gm_av) + 
  geom_line(aes(month, transit), group = 1) + 
  guides(x = guide_axis(angle = 90))
transit

#workplaces
workplaces <- ggplot(gm_av) +
  geom_line(aes(month, workplaces), group = 1) + 
  guides(x = guide_axis(angle = 90))
workplaces

#plot residential
residential <- ggplot(gm_av) +
  geom_line(aes(month, residential), group = 1) + 
  guides(x = guide_axis(angle = 90))
residential
