...
gam_o1 <- gam(other ~ s(predictor), data = mob_cnt, method = "REML")
...
#model other data using GAM
gam_o2 <- gam(other ~ s(predictor), data = mob_cnt, method = "REML")
...
#create predictor for 'other' contacts
mob_cnt[, predictor := retail * 0.5 + transit * 0.5 + grocery * 0.5 - parks * 0.5]

#model other data using GAM
gam_o3 <- gam(other ~ s(predictor), data = mob_cnt, method = "REML")

#predict using 'new' data
other_p3 = data.table(predictor = seq(0, 1, by = 0.01));
other_p3[, other := pmax(0.0, predict(gam_o3, other_p3, type = "response"))]

#plot
plo3 = ggplot(mob_cnt) + 
  geom_point(aes(x = predictor, y = other, col = status)) + 
  geom_line(data = other_p3, aes(x = predictor, y = other)) + 
  labs(x = "Google Mobility weighted 'transit stations', 'parks', \n'retail and recreation', and 'grocery and pharmacy' visits", 
       y = "Number of 'other' contacts", colour = "Status")
plo3
...
#model other data using GAM
gam_o1 <- gam(other ~ s(predictor, k = 3), data = mob_cnt, method = "REML")
...
#model other data using GAM
gam_o2 <- gam(other ~ s(predictor, k = 3), data = mob_cnt, method = "REML")
...
#model other data using GAM
gam_o3 <- gam(other ~ s(predictor, k = 3), data = mob_cnt, method = "REML")
