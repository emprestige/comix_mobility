...
cnts2 <- cnts
cnts2[, social_prop := ifelse(part_social_group == "A - Upper middle class",
...
cnts2[, social_pop := social_prop*67330000]

surv_des <- svydesign(ids = ~cnts2$part_id, strata = ~cnts2$part_social_group, fpc = ~cnts2$social_pop, data = cnts2, nest = T)
weights <- weights(surv_des, type = "sampling")
cnts3 <- cnts2 
cnts3$social_weight <- weights

# social2 <- cnts3[, .(n = .N), by = .(social_weight, week)][, freq := prop.table(n), by = week]
# ggplot(data = social2, aes(x = week, y = freq, fill = social_weight)) + geom_col() + 
#   scale_x_discrete(breaks = my_list)
...
num <- cnts_date[, .(date, part_id, panel, part_age, survey_round, weekday, 
                     ...
                     other = n_cnt_other, all = n_cnt, social_weight)]
...
#model work data using linear regression
lm_w5 <- lm(work ~ poly(workplaces, 2) + school_closure + public_info, 
            data = mob_cnt_train)
summary(lm_w5)
...
lm_h4 <- lm(nonhome ~ poly(residential, 2) + work_closure + school_closure +
              public_info, data = mob_cnt_train)
summary(lm_h4)
