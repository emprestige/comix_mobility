...
gm2 <- gm2[, .(date, study, retail_recreation, grocery_pharmacy, 
...
mob_cnt <- mob_cnt[, .(week, study, status, work, other, nonhome,
                       workplaces, retail, grocery, transit, residential)]

#model work data using GAM - minimum k for smoothing term
gam_w1 <- gam(work ~ s(workplaces, k = 3), data = mob_cnt, method = "REML")
...
gam_w2 <- gam(work ~ s(workplaces, k = 7), data = mob_cnt, method = "REML")

#predict using 'new' data
work_p2 = data.table(workplaces = seq(0, 1, by = 0.01));
...
#model work data using GAM 
gam_w3 <- gam(work ~ s(workplaces), data = mob_cnt, method = "REML")

#predict using 'new' data
work_p3 = data.table(workplaces = seq(0, 1, by = 0.01));
work_p3[, work := pmax(0.0, predict(gam_w3, work_p3, type = "response"))]

#plot
plw3 = ggplot(mob_cnt) + 
  geom_point(aes(x = workplaces, y = work, col = status)) + 
  geom_line(data = work_p3, aes(x = workplaces, y = work)) +
  labs(x = "Google Mobility\n'workplaces' visits", col = "Status",
       y = "Number of work contacts")
plw3
