...
#import contact data
cnts <- qs::qread(file.path(data_path, "cnts_weight_test2.qs"))
...
#model non-home data using linear regression
lm_h4 <- lm(nonhome ~ residential + school_closure + public_info, data = mob_cnt_train)
summary(lm_h4)
...
#model non-home data using polynomial regression
lm_h5 <- lm(nonhome ~ poly(residential, 2) + school_closure + public_info, 
            data = mob_cnt_train)
summary(lm_h5)
