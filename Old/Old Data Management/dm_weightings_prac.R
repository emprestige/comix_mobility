...
#import contact data
pdt <- qs::qread(file.path(data_path, "part_cnts.qs"))
...
pdt[, part_gender := part_gender_nb]
...
pop2[, social_group := list("A - Upper middle class", "B - Middle class",
       "C1 - Lower middle class", "C2 - Skilled working class", 
       "D - Working class", "E - Lower level of substince")]
pop2[, social_proportion := ifelse(part_social_group == "A - Upper middle class",
...
pop3 <- pop2[, .(pop_estimate = sum(estimate)), by = c("part_age_group", "part_gender")]
pop3 <- pop3[, pop_total := sum(pop_estimate), by = c("part_gender")]
...
weightlookup <- pdt[!is.na(part_age_group) & part_gender != "other", 
                    .(sample = .N), by = .(week, part_gender, part_age_group)]
weightlookup[, sample_total := sum(sample), by = .(part_gender, week)]
weightlookup[, sample_proportion := sample / sample_total]

weightlookupage <- pdt[!is.na(part_age_group), .(sample = .N), 
                       by = .(country, mid_date, part_age_group, sample_type)]
