...
gam_w <- gam(work ~ s(workplaces, k = 5), data = mob_cnt_train, method = "REML")
...
fit1 <- gam(work ~ s(workplaces, by = status, k = 3) + status, data = nopoly, 
              method = "REML")
...
#model work data using GAM 
gam_w2 <- gam(work ~ s(workplaces, k = 3) + s(stringency_index),
...
#model work data using GAM 
gam_o <- gam(other ~ s(predictor, k = 4), data = mob_cnt_train, 
             method = "REML")
...
#model other data using GAM
gam_o2 <- gam(other ~ s(predictor, k = 4) + s(stringency_index), 
              data = mob_cnt_train)
