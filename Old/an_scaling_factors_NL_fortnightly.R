...
weighted_means <- num_merge[, .(study, work = weighted.mean(work, day_weight),
                  ...
                  by = .(fortnight = paste(isoyear(date), "/", sprintf("%02d", 
...
gm <- gm2[, .(workplaces = mean(workplaces),
          ...
          by = .(fortnight = ifelse(study == "CoMix",
                 paste(year(date), "/", sprintf("%02d", ceiling(isoweek(date)/2))),
...
#merge
mob_cnt <- merge(weighted_means, gm, by = c("fortnight", "study"))

#calculate scaling factor
ests <- rlang::duplicate(mob_cnt)
ests <- ests[study == "CoMix"]
ests <- ests[, .(fortnight, work, work_mob = workplaces, other, other_mob = predictor)]
ests[, work_mob2 := work_mob**2]
ests[, other_mob2 := other_mob**2]
ests[, work_scaling_fac_lin := (-0.3965 + 2.2126*work_mob)/1.9529120]
ests[, work_scaling_fac_quad := (1.1187 -4.0500*work_mob + 6.0459*work_mob2)/1.9529120]
ests[, other_scaling_fac_lin := (-0.2708 + 1.6202*other_mob)/3.4837340]
ests[, other_scaling_fac_quad := (2.7287 - 8.7273*other_mob + 8.5987*other_mob2)/3.4837340]

#save scaling factors 
qs::qsave(ests, file.path(data_path, "scaling_factors_NL_fortnightly.qs"))
write.csv(ests, file.path(data_path, "scaling_factors_NL_fortnightly.csv"))
