...
#import contact data
cnts <- qs::qread(file.path(data_path, "cnts_weight_other.qs"))
...
weighted_means <- num_merge[, .(study, status, special, 
                  ...
                  by = .(fortnight = paste(isoyear(date), "/", sprintf("%02d", ceiling(isoweek(date)/2))))]  
...
mob_cnt <- mob_cnt[order(fortnight)]
mob_cnt <- mob_cnt[, .(fortnight, study, status, special, work, other, 
