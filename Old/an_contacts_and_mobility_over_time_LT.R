...
#import contact data
cnts <- qs::qread(file.path(data_path, "part_cnts_LT.qs"))
...
L1 <- interval(ymd("2020-03-16"), ymd("2020-06-16")) #first lockdown
T2 <- interval(ymd("2020-06-17"), ymd("2020-11-03")) #post first lockdown
L2 <- interval(ymd("2020-11-04"), ymd("2021-07-01")) #second lockdown
T3 <- interval(ymd("2021-07-02"), ymd("2022-05-01")) #post second lockdown
...
lockdowns$status <- ifelse(ymd(lockdowns$date) %within% T1, 1,
                           ...
                           ifelse(ymd(lockdowns$date) %within% T3, 1, 0)))))
...
mob <- qs::qread(file.path(data_path, "google_mob_LT.qs"))

#subset for same date range
mob_sub <- mob[date >= "2021-01-24" & date <= "2021-09-29"]
