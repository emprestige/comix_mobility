...
mob_cnt <- mob_cnt[, .(week, study, status, stringency_index,
                       work, other, nonhome,
...
gam_w1 <- gam(work ~ s(workplaces, by = status) + status, data = mob_cnt, 
              method = "REML")

#predict using 'new' data
work_p1 = data.table(workplaces = seq(0, 1, by = 0.01),
                     status = ifelse(runif(101, 0, 1) < 0.25, 0, 
                              ifelse(runif(101, 0, 1) > 0.25 & 
                                       runif(101, 0, 1) < 0.5, 1,
                              ifelse(runif(101, 0, 1) > 0.5 & 
                                       runif(101, 0, 1) < 0.75, 2, 3))))
work_p1$status <- factor(work_p1$status, levels = c(0, 1, 2, 3), 
                         labels = c("No Restrictions", "Some Restrictions",
                                  "Lockdown", "Pre-Pandemic"))
...
#model work data using GAM 
gam_w2 <- gam(work ~ s(workplaces) + s(stringency_index), data = mob_cnt, 
              method = "REML")
...
gam_h2 <- gam(nonhome ~ s(residential), data = mob_cnt, method = "REML")

#predict using 'new' data
nonhome_p2 = data.table(residential = seq(1, 1.3, by = 0.01));
