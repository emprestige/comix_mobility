...
#model work data using GAM - 4 (k-1) d.f. for smoothing term
gam_w2 <- gam(work ~ s(workplaces), data = mob_cnt, method = "REML")
...
gam_o1 <- gam(other ~ s(predictor, k = 3), data = mob_cnt)
...
#model other data using GAM
gam_o2 <- gam(other ~ s(predictor), data = mob_cnt)
...
gam_h1 <- gam(nonhome ~ s(residential, k = 3), data = mob_cnt, method = "REML")
