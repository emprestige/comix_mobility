...
contact_matrices <- qs::qread(file.path(data_path, "data", "contact_matrices_NL.qs"))
scaling_factors <- qs::qread(file.path(data_path, "data", "scaling_factors_NL_fortnightly_filtered_middate_longer.qs"))
...
eigens <- qs::qread(file.path(data_path, "data", "comix_eigens_work_NL_fortnightly_filtered_middate_longer.qs"))
...
work <- ggplot(data = reproduction_all) + 
  ...
  labs(x = "Date", y = "Partial Reproduction Number (''Work'')", colour = "Estimate Type") + 
  scale_x_date(labels = date_format("%b-%y"), breaks = "2 months") + ylim(0, 1.1) +
...
#import dominant eigenvalues from comix matrices 
eigens <- qs::qread(file.path(data_path, "data", "comix_eigens_other_NL_fortnightly_filtered_middate_longer.qs"))
...
other <- ggplot(data = reproduction_all) + 
  ...
  geom_line(aes(x = mid_date, y = reproduction_number_lin, col = "lin"), group = 1, size = 0.8) +
  geom_line(aes(x = mid_date, y = reproduction_number_quad, col = "quad"), group = 1, size = 0.8) +
  labs(x = "Date", y = "Partial Reproduction Number (''Other'')", colour = "Estimate Type") +
  scale_x_date(labels = date_format("%b-%y"), breaks = "2 months") + ylim(0, 1.1) +
  scale_color_manual(breaks = c("comix", "mob", "mob2", "lin", "quad"),
                     values = c("#009E73", "#CC79A7", "#D55E00", "#0072B2", "#F0E442"),
                     labels = c("CoMix", "Mobility", "Mobility Squared", 
