...
scaling_factors <- qs::qread(file.path(data_path, "scaling_factors_BE_fortnightly.qs"))
...
eigens <- qs::qread(file.path(data_path, "comix_eigens_other_BE_archive_fortnightly.qs"))
...
p1 <- ggplot(data = eigens_all, aes(x = dominant_eigenvalue)) + 
  ...
  labs(x = "Dominant Eigenvalue (CoMix)", y = "Dominant Eigenvalue (Estimates)",
       colour = "Scaling Factor") + xlim(0, 5) + ylim(0, 5) + 
...
ggplot(data = eigens_all) + 
  ...
  labs(x = "Fortnight", y = "Dominant Eigenvalue", colour = "Estimate Type") +
