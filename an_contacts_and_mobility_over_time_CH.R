...
#import contact data
cnts <- qs::qread(file.path(data_path, "part_cnts_CH.qs"))
...
T1 <- interval(ymd("2020-03-02"), ymd("2020-03-19")) #prior to first lockdown
L1 <- interval(ymd("2020-03-20"), ymd("2020-04-26")) #first lockdown
T2 <- interval(ymd("2020-04-27"), ymd("2020-12-14")) #post first lockdown
L2 <- interval(ymd("2020-12-15"), ymd("2021-03-01")) #second lockdown
T3 <- interval(ymd("2021-03-02"), ymd("2022-02-16")) #post second lockdown
F1 <- interval(ymd("2022-02-17"), ymd("2022-03-02")) #lifted restrictions
...
lockdowns$status <- ifelse(ymd(lockdowns$date) %within% T1, 1,
                           ...
                           ifelse(ymd(lockdowns$date) %within% F1, 1, 0))))))
...
weighted_date <- num_merge[, .(study, special, status,
...
mob <- qs::qread(file.path(data_path, "google_mob_CH.qs"))

#subset for same date range
mob_sub <- mob[date >= "2021-01-21" & date <= "2021-09-14"]
