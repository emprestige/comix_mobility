...
ox <- qs::qread(file.path(data_path, "stringency_component_NL.qs"))
...
lm_w2 <- lm(work ~ poly(workplaces, 2) + work_closure + school_closure + #public_info +
              internal_movement, data = mob_cnt_train)
summary(lm_w2)
...
#model work data using linear regression
lm_w3 <- lm(work ~ workplaces + internal_movement, data = mob_cnt_train)
summary(lm_w3)
...
#model work data using polynomial regression
lm_w4 <- lm(work ~ poly(workplaces, 2) + internal_movement, data = mob_cnt_train)
summary(lm_w4)
...
lm_h1 <- lm(nonhome ~ residential + work_closure + school_closure + 
              internal_movement, data = mob_cnt_train)
summary(lm_h1)
...
#model non-home data using linear regression
lm_h3 <- lm(nonhome ~ residential + internal_movement, data = mob_cnt_train)
summary(lm_h3)
...
#model non-home data using polynomial regression
lm_h4 <- lm(nonhome ~ poly(residential, 2) + internal_movement, 
            data = mob_cnt_train)
summary(lm_h4)
...
lm_o1 <- lm(other ~ predictor + work_closure + school_closure +
              internal_movement, data = mob_cnt_train)
summary(lm_o1)
...
#model 'other' data using linear regression 
lm_o3 <- lm(other ~ predictor + work_closure + internal_movement, 
            data = mob_cnt_train)
summary(lm_o3)
...
#model 'other' data using polynomial regression 
lm_o4 <- lm(other ~ poly(predictor, 2) + internal_movement, data = mob_cnt_train)
summary(lm_o4)
