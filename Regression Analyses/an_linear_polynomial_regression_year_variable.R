...
lm_w1 <- lm(work ~ workplaces*p_year, data = mob_cnt)
summary(lm_w1)

#plot interaction
interact_plot(lm_w1, pred = workplaces, modx = p_year, plot.points = T, 
              data = mob_cnt, modx.values = c(1, 2))

#predict using 'new' data (one for each pandemic year)
mob_cnt1 <- rlang::duplicate(mob_cnt)
mob_cnt2 <- rlang::duplicate(mob_cnt)
mob_cnt1[, p_year := 1]
mob_cnt2[, p_year := 2]

#predict using both data sets
pred1 <- predict(lm_w1, newdata = mob_cnt1, interval = "confidence")
pred1 <- as.data.table(pred1)
mob_cnt1[, work := pred1$fit]
mob_cnt1[, w_lwr := pred1$lwr]
mob_cnt1[, w_uppr := pred1$upr]
pred2 <- predict(lm_w1, newdata = mob_cnt2, interval = "confidence")
pred2 <- as.data.table(pred2)
mob_cnt2[, work := pred2$fit]
mob_cnt2[, w_lwr := pred2$lwr]
mob_cnt2[, w_uppr := pred2$upr]

#plot
plw1 = ggplot(mob_cnt, aes(x = workplaces, y = work, label = special)) +
  geom_point(aes(col = status, shape = factor(p_year))) + geom_text_repel(size = 2.5) +
  geom_line(data = mob_cnt1, aes(x = workplaces, y = work, linetype = "Year 1")) +
  geom_ribbon(data = mob_cnt1, aes(ymin = w_lwr, ymax = w_uppr), alpha = 0.1) +
  geom_line(data = mob_cnt2, aes(x = workplaces, y = work, linetype = "Year 2")) +
...
lm_w2 <- lm(work ~ poly(workplaces, 2, raw = T)*p_year, data = mob_cnt)
summary(lm_w2)

#plot interaction
interact_plot(lm_w2, pred = workplaces, modx = p_year, plot.points = T, data = mob_cnt)
...
#predict using both data sets
pred1 <- predict(lm_w2, newdata = mob_cnt1, interval = "confidence")
pred1 <- as.data.table(pred1)
mob_cnt1[, work := pred1$fit]
mob_cnt1[, w_lwr := pred1$lwr]
mob_cnt1[, w_uppr := pred1$upr]
pred2 <- predict(lm_w2, newdata = mob_cnt2, interval = "confidence")
pred2 <- as.data.table(pred2)
mob_cnt2[, work := pred2$fit]
mob_cnt2[, w_lwr := pred2$lwr]
mob_cnt2[, w_uppr := pred2$upr]
...
lm_w3 <- lm(work ~ workplaces*factor(p_year), data = mob_cnt)
summary(lm_w3)

#plot interaction
interact_plot(lm_w3, pred = workplaces, modx = p_year, plot.points = T, data = mob_cnt)
...
#predict using both data sets
pred1 <- predict(lm_w3, newdata = mob_cnt1, interval = "confidence")
pred1 <- as.data.table(pred1)
mob_cnt1[, work := pred1$fit]
mob_cnt1[, w_lwr := pred1$lwr]
mob_cnt1[, w_uppr := pred1$upr]
pred2 <- predict(lm_w3, newdata = mob_cnt2, interval = "confidence")
pred2 <- as.data.table(pred2)
mob_cnt2[, work := pred2$fit]
mob_cnt2[, w_lwr := pred2$lwr]
mob_cnt2[, w_uppr := pred2$upr]
...
#plot
plw3 = ggplot(mob_cnt, aes(x = workplaces, y = work, label = special)) +
  geom_point(aes(col = status, shape = factor(p_year))) + geom_text_repel(size = 2.5) +
  geom_line(data = mob_cnt1, aes(x = workplaces, y = work, linetype = "Year 1")) +
  geom_ribbon(data = mob_cnt1, aes(ymin = w_lwr, ymax = w_uppr), alpha = 0.1) +
  geom_line(data = mob_cnt2, aes(x = workplaces, y = work, linetype = "Year 2")) +
...
#predict using both data sets
pred1 <- predict(lm_w4, newdata = mob_cnt1, interval = "confidence")
pred1 <- as.data.table(pred1)
mob_cnt1[, work := pred1$fit]
mob_cnt1[, w_lwr := pred1$lwr]
mob_cnt1[, w_uppr := pred1$upr]
pred2 <- predict(lm_w4, newdata = mob_cnt2, interval = "confidence")
pred2 <- as.data.table(pred2)
mob_cnt2[, work := pred2$fit]
mob_cnt2[, w_lwr := pred2$lwr]
mob_cnt2[, w_uppr := pred2$upr]
...
#plot
plw4 = ggplot(mob_cnt, aes(x = workplaces, y = work, label = special)) +
  geom_point(aes(col = status, shape = factor(p_year))) + geom_text_repel(size = 2.5) +
  geom_line(data = mob_cnt1, aes(x = workplaces, y = work, linetype = "Year 1")) +
  geom_ribbon(data = mob_cnt1, aes(ymin = w_lwr, ymax = w_uppr), alpha = 0.1) +
  geom_line(data = mob_cnt2, aes(x = workplaces, y = work, linetype = "Year 2")) +
...
#linear regression including interaction term for pandemic year
lm_h1 <- lm(nonhome ~ residential*p_year, data = mob_cnt)
summary(lm_h1)

#plot interaction
interact_plot(lm_h1, pred = residential, modx = p_year, data = mob_cnt, plot.points = T)
...
#predict using both data sets
pred1 <- predict(lm_h1, newdata = mob_cnt1, interval = "confidence")
pred1 <- as.data.table(pred1)
mob_cnt1[, nonhome := pred1$fit]
mob_cnt1[, h_lwr := pred1$lwr]
mob_cnt1[, h_uppr := pred1$upr]
pred2 <- predict(lm_h1, newdata = mob_cnt2, interval = "confidence")
pred2 <- as.data.table(pred2)
mob_cnt2[, nonhome := pred2$fit]
mob_cnt2[, h_lwr := pred2$lwr]
mob_cnt2[, h_uppr := pred2$upr]

#plot
plh1 = ggplot(mob_cnt, aes(x = residential, y = nonhome, label = special)) +
  geom_point(aes(col = status, shape = factor(p_year))) + geom_text_repel(size = 2.5) +
  geom_line(data = mob_cnt1, aes(x = residential, y = nonhome, linetype = "Year 1")) +
  geom_ribbon(data = mob_cnt1, aes(ymin = h_lwr, ymax = h_uppr), alpha = 0.1) +
  geom_line(data = mob_cnt2, aes(x = residential, y = nonhome, linetype = "Year 2")) +
  geom_ribbon(data = mob_cnt2, aes(ymin = h_lwr, ymax = h_uppr), alpha = 0.1) +
...
#quadratic regression including interaction term for pandemic year
lm_h2 <- lm(nonhome ~ poly(residential, 2, raw = T)*p_year, data = mob_cnt)
summary(lm_h2)

#plot interaction
interact_plot(lm_h2, pred = residential, modx = p_year, data = mob_cnt, plot.points = T)
...
#predict using both data sets
pred1 <- predict(lm_h2, newdata = mob_cnt1, interval = "confidence")
pred1 <- as.data.table(pred1)
mob_cnt1[, nonhome := pred1$fit]
mob_cnt1[, h_lwr := pred1$lwr]
mob_cnt1[, h_uppr := pred1$upr]
pred2 <- predict(lm_h2, newdata = mob_cnt2, interval = "confidence")
pred2 <- as.data.table(pred2)
mob_cnt2[, nonhome := pred2$fit]
mob_cnt2[, h_lwr := pred2$lwr]
mob_cnt2[, h_uppr := pred2$upr]
...
#plot
plh2 = ggplot(mob_cnt, aes(x = residential, y = nonhome, label = special)) +
  geom_point(aes(col = status, shape = factor(p_year))) + geom_text_repel(size = 2.5) +
  geom_line(data = mob_cnt1, aes(x = residential, y = nonhome, linetype = "Year 1")) +
  geom_ribbon(data = mob_cnt1, aes(ymin = h_lwr, ymax = h_uppr), alpha = 0.1) +
  geom_line(data = mob_cnt2, aes(x = residential, y = nonhome, linetype = "Year 2")) +
...
#linear regression including interaction term for pandemic year
lm_h3 <- lm(nonhome ~ residential*factor(p_year), data = mob_cnt)
summary(lm_h3)

#plot interaction
interact_plot(lm_h3, pred = residential, modx = p_year, data = mob_cnt, plot.points = T)
...
#predict using both data sets
pred1 <- predict(lm_h3, newdata = mob_cnt1, interval = "confidence")
pred1 <- as.data.table(pred1)
mob_cnt1[, nonhome := pred1$fit]
mob_cnt1[, h_lwr := pred1$lwr]
mob_cnt1[, h_uppr := pred1$upr]
pred2 <- predict(lm_h3, newdata = mob_cnt2, interval = "confidence")
pred2 <- as.data.table(pred2)
mob_cnt2[, nonhome := pred2$fit]
mob_cnt2[, h_lwr := pred2$lwr]
mob_cnt2[, h_uppr := pred2$upr]
...
#plot
plh3 = ggplot(mob_cnt, aes(x = residential, y = nonhome, label = special)) +
  geom_point(aes(col = status, shape = factor(p_year))) + geom_text_repel(size = 2.5) +
  geom_line(data = mob_cnt1, aes(x = residential, y = nonhome, linetype = "Year 1")) +
  geom_ribbon(data = mob_cnt1, aes(ymin = h_lwr, ymax = h_uppr), alpha = 0.1) +
  geom_line(data = mob_cnt2, aes(x = residential, y = nonhome, linetype = "Year 2")) +
...
#predict using both data sets
pred1 <- predict(lm_h4, newdata = mob_cnt1, interval = "confidence")
pred1 <- as.data.table(pred1)
mob_cnt1[, nonhome := pred1$fit]
mob_cnt1[, h_lwr := pred1$lwr]
mob_cnt1[, h_uppr := pred1$upr]
pred2 <- predict(lm_h4, newdata = mob_cnt2, interval = "confidence")
pred2 <- as.data.table(pred2)
mob_cnt2[, nonhome := pred2$fit]
mob_cnt2[, h_lwr := pred2$lwr]
mob_cnt2[, h_uppr := pred2$upr]
...
#plot
plh4 = ggplot(mob_cnt, aes(x = residential, y = nonhome, label = special)) +
  geom_point(aes(col = status, shape = factor(p_year))) + geom_text_repel(size = 2.5) +
  geom_line(data = mob_cnt1, aes(x = residential, y = nonhome, linetype = "Year 1")) +
  geom_ribbon(data = mob_cnt1, aes(ymin = h_lwr, ymax = h_uppr), alpha = 0.1) +
  geom_line(data = mob_cnt2, aes(x = residential, y = nonhome, linetype = "Year 2")) +
...
#linear regression including interaction term for pandemic year
lm_o1 <- lm(other ~ predictor*p_year, data = mob_cnt)
summary(lm_o1)

#plot interaction
interact_plot(lm_o1, pred = predictor, modx = p_year, data = mob_cnt, plot.points = T)
...
#predict using both data sets
pred1 <- predict(lm_o1, newdata = mob_cnt1, interval = "confidence")
pred1 <- as.data.table(pred1)
mob_cnt1[, other := pred1$fit]
mob_cnt1[, o_lwr := pred1$lwr]
mob_cnt1[, o_uppr := pred1$upr]
pred2 <- predict(lm_o1, newdata = mob_cnt2, interval = "confidence")
pred2 <- as.data.table(pred2)
mob_cnt2[, other := pred2$fit]
mob_cnt2[, o_lwr := pred2$lwr]
mob_cnt2[, o_uppr := pred2$upr]

#plot
plo1 = ggplot(mob_cnt, aes(x = predictor, y = other, label = special)) +
  geom_point(aes(col = status, shape = factor(p_year))) + geom_text_repel(size = 2.5) +
  geom_line(data = mob_cnt1, aes(x = predictor, y = other, linetype = "Year 1")) +
  geom_ribbon(data = mob_cnt1, aes(ymin = o_lwr, ymax = o_uppr), alpha = 0.1) +
  geom_line(data = mob_cnt2, aes(x = predictor, y = other, linetype = "Year 2")) +
  geom_ribbon(data = mob_cnt2, aes(ymin = o_lwr, ymax = o_uppr), alpha = 0.1) +
...
#quadratic regression including interaction term for pandemic year
lm_o2 <- lm(other ~ poly(predictor, 2, raw = T)*p_year, data = mob_cnt)
summary(lm_o2)

#plot interaction
interact_plot(lm_o2, pred = predictor, modx = p_year, data = mob_cnt, plot.points = T)
...
#predict using both data sets
pred1 <- predict(lm_o2, newdata = mob_cnt1, interval = "confidence")
pred1 <- as.data.table(pred1)
mob_cnt1[, other := pred1$fit]
mob_cnt1[, o_lwr := pred1$lwr]
mob_cnt1[, o_uppr := pred1$upr]
pred2 <- predict(lm_o2, newdata = mob_cnt2, interval = "confidence")
pred2 <- as.data.table(pred2)
mob_cnt2[, other := pred2$fit]
mob_cnt2[, o_lwr := pred2$lwr]
mob_cnt2[, o_uppr := pred2$upr]
...
#linear regression including interaction term for pandemic year
lm_o3 <- lm(other ~ predictor*factor(p_year), data = mob_cnt)
summary(lm_o3)

#plot interaction
interact_plot(lm_o3, pred = predictor, modx = p_year, data = mob_cnt, plot.points = T)
...
#predict using both data sets
pred1 <- predict(lm_o3, newdata = mob_cnt1, interval = "confidence")
pred1 <- as.data.table(pred1)
mob_cnt1[, other := pred1$fit]
mob_cnt1[, o_lwr := pred1$lwr]
mob_cnt1[, o_uppr := pred1$upr]
pred2 <- predict(lm_o3, newdata = mob_cnt2, interval = "confidence")
pred2 <- as.data.table(pred2)
mob_cnt2[, other := pred2$fit]
mob_cnt2[, o_lwr := pred2$lwr]
mob_cnt2[, o_uppr := pred2$upr]
...
#plot
plo3 = ggplot(mob_cnt, aes(x = predictor, y = other, label = special)) +
  geom_point(aes(col = status, shape = factor(p_year))) + geom_text_repel(size = 2.5) +
  geom_line(data = mob_cnt1, aes(x = predictor, y = other, linetype = "Year 1")) +
  geom_ribbon(data = mob_cnt1, aes(ymin = o_lwr, ymax = o_uppr), alpha = 0.1) +
  geom_line(data = mob_cnt2, aes(x = predictor, y = other, linetype = "Year 2")) +
...
#predict using both data sets
pred1 <- predict(lm_o4, newdata = mob_cnt1, interval = "confidence")
pred1 <- as.data.table(pred1)
mob_cnt1[, other := pred1$fit]
mob_cnt1[, o_lwr := pred1$lwr]
mob_cnt1[, o_uppr := pred1$upr]
pred2 <- predict(lm_o4, newdata = mob_cnt2, interval = "confidence")
pred2 <- as.data.table(pred2)
mob_cnt2[, other := pred2$fit]
mob_cnt2[, o_lwr := pred2$lwr]
mob_cnt2[, o_uppr := pred2$upr]
...
#plot
plo4 = ggplot(mob_cnt, aes(x = predictor, y = other, label = special)) +
  geom_point(aes(col = status, shape = factor(p_year))) + geom_text_repel(size = 2.5) +
  geom_line(data = mob_cnt1, aes(x = predictor, y = other, linetype = "Year 1")) +
  geom_ribbon(data = mob_cnt1, aes(ymin = o_lwr, ymax = o_uppr), alpha = 0.1) +
  geom_line(data = mob_cnt2, aes(x = predictor, y = other, linetype = "Year 2")) +
