...
#model work data using linear regression
lm_w3 <- lm(work ~ workplaces + work_closure + school_closure + public_info + 
...
#model work data using polynomial regression
lm_w4 <- lm(work ~ poly(workplaces, 2) + work_closure + school_closure + 
...
#model work data using linear regression
lm_w5 <- lm(work ~ workplaces + work_closure + public_info + internal_movement, 
            data = mob_cnt_train)
summary(lm_w5)
...
#model work data using polynomial regression
lm_w6 <- lm(work ~ poly(workplaces, 2) + work_closure + public_info + 
              internal_movement, data = mob_cnt_train)
summary(lm_w6)
...
#model non-home data using linear regression
lm_h3 <- lm(nonhome ~ residential + work_closure + school_closure + public_info +
              internal_movement, data = mob_cnt_train)
summary(lm_h3)
...
#model non-home data using polynomial regression
lm_h4 <- lm(nonhome ~ poly(residential, 2) + work_closure + school_closure +
              public_info + internal_movement, data = mob_cnt_train)
summary(lm_h4)
...
#model non-home data using linear regression
lm_h5 <- lm(nonhome ~ residential + public_info, data = mob_cnt_train)
summary(lm_h5)

#predict using 'new' data
pred <- predict(lm_h5, newdata = mob_cnt_test, interval = "confidence")
pred <- as.data.table(pred)
mob_cnt_test[, nonhome := pred$fit]
mob_cnt_test[, h_lwr := pred$lwr]
mob_cnt_test[, h_uppr := pred$upr]
...
#plot
plh5 = ggplot(mob_cnt_train, aes(x = residential, y = nonhome,
          label = ifelse(status == "No restrictions", week, special))) +
  geom_point(aes(col = status)) + geom_text_repel(size = 2.5, max.overlaps = 100) +
  geom_line(data = mob_cnt_test, aes(x = residential, y = nonhome)) +
  geom_ribbon(data = mob_cnt_test, aes(ymin = h_lwr, ymax = h_uppr), alpha = 0.1) +
...
#model non-home data using polynomial regression
lm_h6 <- lm(nonhome ~ poly(residential, 2) + public_info + internal_movement, 
            data = mob_cnt_train)
summary(lm_h6)

#predict using 'new' data
pred <- predict(lm_h6, newdata = mob_cnt_test, interval = "confidence")
pred <- as.data.table(pred)
mob_cnt_test[, nonhome := pred$fit]
mob_cnt_test[, h_lwr := pred$lwr]
mob_cnt_test[, h_uppr := pred$upr]
...
#plot
plh6 = ggplot(mob_cnt_train, aes(x = residential, y = nonhome,
          label = ifelse(status == "No restrictions", week, special))) +
  geom_point(aes(col = status)) + geom_text_repel(size = 2.5, max.overlaps = 100) +
  geom_line(data = mob_cnt_test, aes(x = residential, y = nonhome)) +
  geom_ribbon(data = mob_cnt_test, aes(ymin = h_lwr, ymax = h_uppr), alpha = 0.1) +
...
#model 'other' data using linear regression 
lm_o3 <- lm(other ~ predictor + work_closure + school_closure + public_info +
              internal_movement, data = mob_cnt_train)
summary(lm_o3)
# 
# #predict using 'new' data
...
#model 'other' data using polynomial regression 
lm_o4 <- lm(other ~ poly(predictor, 2) + work_closure + school_closure + 
              public_info + internal_movement, data = mob_cnt_train)
summary(lm_o4)
