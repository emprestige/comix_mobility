...
scaling_factors <- qs::qread(file.path(data_path, "scaling_factors_NL_fortnightly_filtered_middate_longer.qs"))
...
eigens <- qs::qread(file.path(data_path, "comix_eigens_work_NL_fortnightly_filtered_middate_longer.qs"))
...
p5 <- ggplot(data = reproduction_all, aes(x = reproduction_number)) + 
  ...
  labs(x = "Reproduction Number (CoMix)", y = "Reproduction Number\n(Estimates)",
       colour = "Scaling Factor") + xlim(0, 0.8) + ylim(0, 0.8) + 
