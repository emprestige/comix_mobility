...
work <- ggplot(data = reproduction_all) + 
  ...
  scale_x_date(labels = date_format("%b-%y"), breaks = "2 months") + ylim(0, 0.5) +
...
#import dominant eigenvalues from comix matrices 
eigens <- qs::qread(file.path(data_path, "comix_eigens_other_UK_fortnightly_filtered_middate.qs"))
