...
cnts <- cnts[sample_type == "child"]

#order by date
cnts_date <- cnts[order(date)]
cnts_date <- cnts_date[, .(date, sample_type, part_age, panel, weekday, 
                           day_weight, survey_round, school = n_cnt_school)]
...
cnts_l <- merge(cnts_date, lockdowns, by = "date", all.y = F)
...
cnt <- num_merge[, .(status, school = weighted.mean(school, day_weight, na.rm = T)),
                 by = .(week = paste(isoyear(date), "/", sprintf("%02d", isoweek(date))))]
cnt <- unique(cnt)

#get labels for all plots
int <- seq(1, 113, 12)
my_list <- cnt$week[int]
ggplot(cnt, aes(x = week, y = school)) + geom_point(aes(col = status)) +
