...
#load data
ox_cgrt <- read.csv(file.path(data_path, "OxCGRT_GBR_latest.csv"))
...
ox_cgrt1 <- ox_cgrt1[, C2M_Workplace.closing := ifelse(C2M_Flag == 1, C2M_Workplace.closing, 0)]
ox_cgrt1 <- ox_cgrt1[, C7M_Restrictions.on.internal.movement := ifelse(C7M_Flag == 1,
                       C7M_Restrictions.on.internal.movement, 0)]
# ox_cgrt1 <- ox_cgrt1[, H1_Public.information.campaigns := ifelse(H1_Public.information.campaigns == 1, 
...
#save
qs::qsave(ox_cgrt1, file.path(data_path, "stringency_component.qs"))
